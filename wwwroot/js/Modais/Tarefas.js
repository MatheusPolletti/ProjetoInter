...
function excluirTarefasSelecionadas() {
    ...
    if (!confirm(`Deseja realmente excluir as ${checkboxes.length} tarefas selecionadas?`)) {
        return;
    }
    ...
    .then(response => {
        if (!response.ok) {
            return response.text().then(text => {
                throw new Error(text || "Erro ao excluir tarefas");
            });
        }
        return response.json();
    })
    .then(data => {
        if (data && data.success) {
            showFeedback(`${data.count} tarefa(s) excluída(s) com sucesso!`, true);
        } else {
            showFeedback(data?.message || "Erro ao excluir tarefas", false);
        }
    })
    .catch(error => {
        console.error("Erro detalhado:", error);
